...
import { USER_STATUS, ORDER_STATUS, PAYMENT_STATUS, TICKET_STATUS, TICKET_PRIORITY } from '../constants/statuses';
...
export const generateCustomers = (count = 50) => {
  ...
  for (let i = 1; i <= count; i++) {
    customers.push({
      ...
      phone: `+1-555-${String(randomInt(1000, 9999))}-${String(randomInt(1000, 9999))}`,
      status: randomItem([USER_STATUS.ACTIVE, USER_STATUS.INACTIVE, USER_STATUS.SUSPENDED]),
      totalOrders: randomInt(0, 50),
      totalSpent: randomInt(100, 5000),
      joinedDate: dayjs().subtract(randomInt(1, 365), 'days').format('YYYY-MM-DD'),
...
export const generateVendors = (count = 30) => {
  const vendors = [];
  const businessNames = ['Fresh Market', 'Tech Store', 'Fashion Hub', 'Home Decor', 'Sports World', 'Beauty Shop', 'Book Corner', 'Pet Paradise'];
  ...
  for (let i = 1; i <= count; i++) {
    ...
    vendors.push({
      ...
      phone: `+1-555-${String(randomInt(1000, 9999))}-${String(randomInt(1000, 9999))}`,
      status: isPending ? USER_STATUS.PENDING : randomItem([USER_STATUS.ACTIVE, USER_STATUS.INACTIVE, USER_STATUS.SUSPENDED]),
      totalSales: randomInt(1000, 50000),
      commission: randomInt(5, 20),
      productsCount: randomInt(5, 200),
      rating: (randomInt(30, 50) / 10).toFixed(1),
      joinedDate: dayjs().subtract(randomInt(1, 730), 'days').format('YYYY-MM-DD'),
...
export const generateDeliveryAgents = (count = 25) => {
  const agents = [];
  const names = ['Agent Smith', 'Agent Brown', 'Agent Wilson', 'Agent Davis', 'Agent Taylor', 'Agent Moore'];
...
export const generateCategories = () => {
  return [
    { id: 1, name: 'Electronics', slug: 'electronics', productsCount: 120, icon: 'laptop', parentId: null },
    { id: 2, name: 'Fashion', slug: 'fashion', productsCount: 250, icon: 'shopping', parentId: null },
    { id: 3, name: 'Home & Garden', slug: 'home-garden', productsCount: 180, icon: 'home', parentId: null },
    { id: 4, name: 'Sports', slug: 'sports', productsCount: 90, icon: 'trophy', parentId: null },
    { id: 5, name: 'Books', slug: 'books', productsCount: 150, icon: 'book', parentId: null },
    { id: 6, name: 'Smartphones', slug: 'smartphones', productsCount: 45, icon: 'mobile', parentId: 1 },
    { id: 7, name: 'Laptops', slug: 'laptops', productsCount: 35, icon: 'laptop', parentId: 1 },
    { id: 8, name: 'Men', slug: 'men', productsCount: 120, icon: 'man', parentId: 2 },
    { id: 9, name: 'Women', slug: 'women', productsCount: 130, icon: 'woman', parentId: 2 },
  ];
};
...
export const generateProducts = (count = 100) => {
  const products = [];
  const productNames = ['Wireless Headphones', 'Smart Watch', 'Laptop', 'T-Shirt', 'Jeans', 'Running Shoes', 'Coffee Maker', 'Book', 'Backpack'];
  ...
  for (let i = 1; i <= count; i++) {
    products.push({
      ...
      createdAt: dayjs().subtract(randomInt(1, 180), 'days').format('YYYY-MM-DD'),
    });
  }
  return products;
};
...
export const generateOrders = (count = 100) => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    orders.push({
      ...
      createdAt: dayjs().subtract(randomInt(0, 90), 'days').format('YYYY-MM-DD HH:mm:ss'),
      updatedAt: dayjs().subtract(randomInt(0, 30), 'days').format('YYYY-MM-DD HH:mm:ss'),
    });
  }
  return orders;
};
...
export const generateTransactions = (count = 100) => {
  ...
  for (let i = 1; i <= count; i++) {
    transactions.push({
      ...
      createdAt: dayjs().subtract(randomInt(0, 180), 'days').format('YYYY-MM-DD HH:mm:ss'),
    });
  }
  return transactions;
};
...
export const generateTickets = (count = 50) => {
  ...
  for (let i = 1; i <= count; i++) {
    tickets.push({
      ...
      createdAt: dayjs().subtract(randomInt(0, 30), 'days').format('YYYY-MM-DD HH:mm:ss'),
      updatedAt: dayjs().subtract(randomInt(0, 5), 'days').format('YYYY-MM-DD HH:mm:ss'),
...
export const generateAuditLogs = (count = 100) => {
  ...
  for (let i = 1; i <= count; i++) {
    logs.push({
      ...
      timestamp: dayjs().subtract(randomInt(0, 30), 'days').format('YYYY-MM-DD HH:mm:ss'),
    });
  }
  return logs;
};
